...
class LBFGSB(base.IterativeSolver):
  ...
  # def nothing(self, cond):
  #   pass

  # def failed(self, cond):
  #   # jax.debug.print("{}, {}", cond, "failed")
  #   pass
  ...
  def _cond_fun(self, inputs):
    ...
    cond = (state.error > self.tol) & jnp.logical_or(
        not self.stop_if_linesearch_fails, ~state.failed_linesearch)
    # jax.lax.cond(cond, self.nothing, self.failed, {"cond":cond})
    return cond
  ...
  def init_state(
    ...
    # jax.debug.print("LBFGSB start", ordered=True)
    # if self.verbose:
    #   jax.debug.print("LBFGSB init_state calling EVAL {}/{}", init_params.shape, init_params[:5], ordered=True)
    (value, aux), grad = self._value_and_grad_with_aux(
        init_params, *args, **kwargs
    )
    if bounds is None and self.bounds:
      bounds = self.bounds
    if bounds is None:
    ...
    # if self.verbose:
    #   jax.debug.print("LBFGSB init_state after EVAL \n bounds:\n{}\n grad:\n{}\nerror\n{}\n", bounds[0][:5,:5], grad[:5, :5], init_error, ordered=True)
    state = LbfgsbState(
        value=value,
        grad=grad,
        error=init_error,
    ...
    #   self.log_info(
    #       state,
    #       error_name="###INIT### Projected Gradient Norm",
    #       additional_info={
    #           "Objective Value": value,
    #           "\nInit Sample": init_params[:5,:5],
    #           "\nStepsize": state_kwargs['stepsize'],
  ...
  def update(
    ...
    Returns:
      (params, state)
    """
    # jax.debug.print("LFBGSB Step {} Started", state.iter_num, ordered=True)
    ...
    if use_linesearch:
      ...
      # if self.verbose:
      #   jax.debug.print("LBFGSB update calling EVAL", ordered=True)
    ...
    #           "\nStepsize": new_stepsize,
    #           "\nGrad": (new_grad.shape, jnp.max(jnp.abs(new_grad))),
    #           "\nAux": (new_aux.shape, jnp.max(new_aux), jnp.min(new_aux), jnp.mean(new_aux), new_aux[:3]),
    #           "\nValue": new_value,
    #           "\nFailed Linesearch?": failed_linesearch,
    #           "\nNumber Linesearch Iterations": 
    #           new_state.num_linesearch_iter - state.num_linesearch_iter
    #       }
    #   )
    #jax.debug.print("LFBGSB Step {} Completed", state.iter_num, ordered=True)
  ...
  def optimality_fun(self, sol, bounds, *args, **kwargs):
    ...
    if bounds is None:
      # if self.verbose:
      #   jax.debug.print("LBFGSB optimality_fun calling EVAL", ordered=True)
