...
def _cond_print(condition, message, **kwargs):
  jax.lax.cond(condition, lambda _: jax.debug.print(message, **kwargs, ordered=True), lambda _: None, None)
...
class ZoomLineSearch(base.IterativeLineSearch):
  ...
  def save_array(self, array):
    print("saving", "./steppedx.npy", array[:5])
    jnp.save("./steppedx.npy", array)
  ...
  def _value_and_slope_on_line(
    ...
    #jax.experimental.io_callback(self.save_array, None, step)
    if self.verbose:
      jax.debug.print("ZOOM val_slope_line calling EVAL", ordered=True)
    ...
    if self.verbose:
      self.log_info(
          self.state,
          error_name="Zoom Value and Slope",
          additional_info={
              "Objective Value": value_step,
              "\nInit Sample": params[0,:],
              "\nAfter Add Scalar": (step[0,:], jnp.max(step), jnp.min(step)),
              "\nDescent Dir?": (descent_direction[0], jnp.max(descent_direction), jnp.min(descent_direction)),
              "\nStepsize": stepsize,
              "\nGrad": (grad_step.shape, jnp.max(jnp.abs(grad_step))),
              "\nAux": (aux_step.shape, jnp.max(aux_step), jnp.min(aux_step), jnp.mean(aux_step), aux_step[0]),
              #"\nValue": new_value,
              #"\nFailed Linesearch?": failed_linesearch,
  ...
  def _make_safe_step(self, stepsize, state, args, kwargs):
    ...
    if self.verbose:
      jax.debug.print("ZOOM descrease error calling EVAL || max step {}", jnp.max(jnp.abs(step)), ordered=True)
    (value_step, aux_step), grad_step = self._value_and_grad_fun_with_aux(
        step, *args, **kwargs
    )
    ...
    if self.verbose:
      self.log_info(
          new_state,
          error_name="Zoom Safe Step",
          additional_info={
              "Objective Value": value_step,
              "\nInit Sample": (jnp.max(jnp.abs(state.params)), state.params[:5,:5]),
              "\nAfter Add Scalar": (jnp.max(jnp.abs(step)), step[:5,:5]),
              "\nStepsize": stepsize,
              "\nGrad": (grad_step.shape, jnp.max(jnp.abs(grad_step))),
              "\nAux": (aux_step.shape, jnp.max(aux_step), jnp.min(aux_step), jnp.mean(aux_step), aux_step[:3]),
              #"\nValue": new_value,
              #"\nFailed Linesearch?": failed_linesearch,
              #"\nNumber Linesearch Iterations": 
              #new_state.num_linesearch_iter - state.num_linesearch_iter
  ...
  def _search_interval(self, init_stepsize, state, args, kwargs):
    ...
    if self.verbose:
      jax.debug.print("ZOOM search_interval cond {}",[low, value_low, slope_low, high, value_high, slope_high],  ordered=True)
      _cond_print(
  ...
  def init_state(
    ...
    aux = None
    if self.verbose:
      jax.debug.print("ZOOM init_state calling EVAL", ordered=True)
    ...
    if aux is None and self.has_aux:
      if self.verbose:
        jax.debug.print("ZOOM init_state calling EVAL just with aux? {}", (value, grad[:5, :5]), ordered=True)
    ...
    self.state = ZoomLineSearchState(
    ...
    if self.verbose:
      self.log_info(
          self.state,
          error_name="Zoom INIT STATE",
          additional_info={
              "Objective Value": value,
              "\nInit Sample": params[:5,:5],
              #"\nAfter LS Sample": new_params[:5,:5],
              #"\nStepsize": new_stepsize,
              "\nGrad": (grad.shape, jnp.max(jnp.abs(grad))),
              "\nAux": (aux.shape, jnp.max(aux), jnp.min(aux), jnp.mean(aux), aux[:3]),
              #"\nValue": new_value,
              #"\nFailed Linesearch?": failed_linesearch,
              #"\nNumber Linesearch Iterations": 
              #new_state.num_linesearch_iter - state.num_linesearch_iter
  ...
  def __post_init__(self):
    if not self.skip_wrap:
      ...
      if self.verbose:
        jax.debug.print("LS Skipping rewrap", ordered=True)
      self._fun_with_aux = self.fun
      self._value_and_grad_fun_with_aux = self.value_and_grad
